...
public class DBLoader : ILoader
{
    ...
    public async Task<IEnumerable<Game>> LoadGameByDateIntervalAndGroup(DateTime startDate, DateTime endDate, Group group, int page, int pageSize)
        => (await ((TarotDBContext)Context ).Games.Where(g => g.StartDate >= startDate && g.EndDate <= endDate && g.Id == group.Id)
                                 .Skip((page-1) * pageSize)
                                 .Take(pageSize)
                                 .ToListAsync()).ToModels();

    public async Task<IEnumerable<Game>> LoadGameByDateIntervalAndPlayer(DateTime startDate, DateTime endDate, Player player, int page,
        int pageSize)
    => (await((TarotDBContext)Context).Games.Where(g => g.StartDate >= startDate && g.EndDate <= endDate && g.Players.Any(p => p.ToModel().Equals(player)))
                                 .Skip((page-1) * pageSize)
                                 .Take(pageSize)
                                 .ToListAsync()).ToModels();
    ...
    public async Task<IEnumerable<Player>> LoadPlayerByLastNameAndNickname(string lastName, string nickname, int page, int pageSize)
        => (await ((TarotDBContext)Context).Players.Where(p => p.LastName.Contains(lastName) && p.Nickname.Contains(nickname))
                                 .Skip((page-1) * pageSize)
                                 .Take(pageSize)
                                 .ToListAsync()).ToModels();

    public async Task<IEnumerable<Player>> LoadPlayerByFirstNameAndNickname(string firstName, string nickname, int page, int pageSize)
        => (await ((TarotDBContext)Context).Players.Where(p => p.FirstName.Contains(firstName) && p.Nickname.Contains(nickname))
                                 .Skip((page-1) * pageSize)
                                 .Take(pageSize)
                                 .ToListAsync()).ToModels();

    public async Task<IEnumerable<Player>> LoadPlayerByFirstNameAndLastName(string firstName, string lastName, int page, int pageSize)
        => (await ((TarotDBContext)Context).Players.Where(p => p.FirstName.Contains(firstName) && p.LastName.Contains(lastName))
                                 .Skip((page-1) * pageSize)
                                 .Take(pageSize)
                                 .ToListAsync()).ToModels();
