...
public class UT_GenericRepositories
{
	...
	public async Task TestConstructor(Type context, Type entity, bool isValid)
	{
		...
		constructor = typeof(GenericRepository<>).MakeGenericType(entity).GetConstructor(new [] {typeof(TarotDbContext)});
		
		object? repository = null;
		if (!isValid)
		{
			var exception = Assert.Throws<TargetInvocationException>(() => constructor!.Invoke(new object?[] {dbContext}));
			var innerException = exception.InnerException;
			Assert.IsType<ArgumentException>(innerException);
			Assert.Equal($"The type {entity.Name} is not a valid entity type for this context.", innerException.Message);
		}
		else
		{
	...
	[Theory]
	[MemberData(nameof(Data))]
	public async Task TestSet(Type context, Type entity, bool isValid)
	{
		if (!isValid) return;
